  ...
  std::vector<double>& cpu_velocity_per_atom)
{
  ...
  if ((step + 1) % dump_interval_ != 0)
    return;
  FILE* fidx = fopen("vx.bin", "a+");
  FILE* fidy = fopen("vy.bin", "a+");
  FILE* fidz = fopen("vz.bin", "a+");
  ...
  if (grouping_method_ < 0) {
    ...
    for (int n = 0; n < num_atoms_total; n++) {
      outfile=cpu_velocity_per_atom[n] * natural_to_A_per_fs;
      fwrite(&outfile,sizeof(double),1,fidx);
      outfile=cpu_velocity_per_atom[n + num_atoms_total]  * natural_to_A_per_fs;
      fwrite(&outfile,sizeof(double),1,fidy);
      outfile=cpu_velocity_per_atom[n + 2 * num_atoms_total] * natural_to_A_per_fs;
      fwrite(&outfile,sizeof(double),1,fidz);
    }
  } else {
